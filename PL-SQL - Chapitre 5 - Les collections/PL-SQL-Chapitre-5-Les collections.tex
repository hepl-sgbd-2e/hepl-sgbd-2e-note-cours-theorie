...
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Exemple 1]{../exemples/PLSQL Chapitre 5/exemple1.sql}
\end{frame}

\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Exemple avec ROWTYPE]{../exemples/PLSQL Chapitre 5/exemple2.sql}
\end{frame}

\subsection{Initialisation}
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Exemple avec initialisation]{../exemples/PLSQL Chapitre 5/exemple3.sql}
\end{frame}

\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Initialiser un tableau associatif]{../exemples/PLSQL Chapitre 5/exemple4.sql}
\end{frame}

\subsection{Méthode exists}
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Tester l'existence ou non d'un élément : méthode exists]{../exemples/PLSQL Chapitre 5/exemple5.sql}
\end{frame}

\subsection{Méthode count}
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Compter le nombre d'éléments d'une collection : méthode count]{../exemples/PLSQL Chapitre 5/exemple6.sql}
\end{frame}

\subsection{Méthode first et last}
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Déterminer le premier et le dernier indice des éléments d'une collection : méthodes first et last]{../exemples/PLSQL Chapitre 5/exemple7.sql}
\end{frame}
...
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=L’indice du dernier élément]{../exemples/PLSQL Chapitre 5/exemple8.sql}
\end{frame}

\subsection{Méthode next}
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Indices non consécutifs]{../exemples/PLSQL Chapitre 5/exemple9.sql}
\end{frame}
...
\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=ORA-06502: PL/SQL : erreur numérique ou erreur sur une valeur]{../exemples/PLSQL Chapitre 5/exemple10.sql}
\end{frame}

\begin{frame}{\secname : \subsecname}
    \lstinputlisting[language=sql, title=Exemple complet : parcourir des résultats de recherches]{../exemples/PLSQL Chapitre 5/exemple11.sql}
\end{frame}
    ...
    \begin{table}[]
        \resizebox{\textwidth}{!}{%
            \begin{tabular}{|l|l|}
                ...
                \lstinline[language=sql]!COLLECTION\_IS\_NULL ! & On utilise une collection atomicaly null                             \\ \hline
                \lstinline[language=sql]!NO\_DATA\_FOUND! & \begin{tabular}[c]{@{}l@{}}Un indice désigne un élément supprimé ou un élément qui n'existe\\ pas dans une table PL/SQL\end{tabular}                                           \\ \hline
                \lstinline[language=sql]!SUBSCRIPT\_BEYOND\_COUNT! & Un indice dépasse le nombre d'éléments de la collection              \\ \hline
                \lstinline[language=sql]!SUBSCRIPT\_OUTSIDE\_LIMIT! & Un indice est en dehors de la fourchette permise                     \\ \hline
                \lstinline[language=sql]!VALUE\_ERROR ! & Un indice est null ou ne peut être converti dans le type de l'indice \\ \hline
            \end{tabular}
        }
    \end{table}
\end{frame}
